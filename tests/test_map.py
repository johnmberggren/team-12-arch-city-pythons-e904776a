...
class TestMap(TestCase):
    ...
    def test_is_position_valid_when_x_too_big(self):
        testobj = Map()
        self.assertFalse(testobj.is_position_valid(Position(100, 4)))

    def test_is_position_valid_when_y_too_small(self):
       testobj = Map()
       self.assertFalse(testobj.is_position_valid(Position(0, -10)))

    def test_is_position_valid_when_y_too_big(self):
        testobj = Map()
        self.assertFalse(testobj.is_position_valid(Position(3, 112)))

    def test_is_position_valid_when_x_and_y_too_big(self):
        testobj = Map()
        self.assertFalse(testobj.is_position_valid(Position(121, 10)))
    ...
    def test_calculate_new_position_when_valid_EAST(self):
        testobj = Map()
        startingPosition = Position(3, 2)
        expectedPosition = Position(4, 2)
